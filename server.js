...
const WordExtractor = require('word-extractor'); // Для старых .doc
...
Все указанные факты о человеке должны проходить простейшую проверку на правдивость
...
async function extractText(filePath, originalName) {
    ...
    try {
        if (ext === '.pdf') {
            ...
            throw new Error("Формат .rtf не поддерживается. Пересохраните как .docx или .pdf.");
        } else {
        ...
        throw new Error("Произошла ошибка при чтении файла. Возможно, файл поврежден.");
    }
}
...
app.post('/api/generate', upload.fields([
    ...
    try {
        ...
        if (needStory) {
            ...
            else userPromptStory += `\n\nВакансия не загружена. Используй позицию из резюме.`;
